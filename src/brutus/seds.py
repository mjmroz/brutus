...
__all__ = ["MISTtracks", "SEDmaker", "FastNN", "FastNNPredictor", "Isochrone"]
...
class MISTtracks(object):
    ...
    def make_lib(self, misth5, verbose=True):
        ...
        try:
            ...
            self.output = [
                np.concatenate([misth5[z][p] for z in misth5["index"]]) for p in cols
            ]
...
class SEDmaker(MISTtracks):
        ...
        if loga <= loga_max:
            ...
            if smf > 0.0 and eep <= eep_binary_max and mini * smf >= mini_min:
                ...
                if eep2 is None:
                    # Convert loga to EEP.
                    eep2 = self.get_eep(
                        loga, mini=mini, eep=eep, feh=feh, smf=smf, tol=tol
                    )
    ...
    def get_eep(self, loga, mini=1.0, eep=350.0, feh=0.0, afe=0.0, smf=1.0, tol=1e-3):
        """
    ...
    def make_grid(
        ...
        **kwargs,
        ...
        for i, (mini, eep, feh, afe, smf) in enumerate(self.grid_label):
            ...
            (sed, params, params2, eep2) = self.get_sed(
                mini=mini,
                eep=eep,
                feh=feh,
                afe=afe,
            ...
            if np.any(np.isnan(sed)) or np.any(np.isnan(params)):
                ...
                self.grid_sed[i] = tuple(np.full((self.FNNP.NFILT, 1), np.nan))
            else:
                ...
                sfits = np.array(
                    [polyfit(av_grid, s, 1, w=av_wt).T for s in seds]
                )  # Av at fixed Rv
                sedr, seda = polyfit(
            ...
            if verbose and new_percentage != percentage:
                ...
                sys.stderr.write(
                    "\rConstructing grid {:6.3f}% ({:d}/{:d}) "
                    "[mini={:6.3f}, eep={:6.3f}, feh={:6.3f}, "
                    "afe={:6.3f}, smf={:6.3f}] "
                    "(t/obj: {:3.3f} ms, "
                    "est. remaining: {:10.3f} s)          ".format(
                        percentage / 1.0e3,
                        i + 1,
                        Ngrid,
                        mini,
...
class Isochrone(object):
    ...
    def build_interpolator(self):
        ...
        self.logl_idx = np.where(np.array(self.predictions) == "logl")[0][0]
        self.logg_idx = np.where(np.array(self.predictions) == "logg")[0][0]
        self.feh_surf_idx = np.where(np.array(self.predictions) == "feh_surf")[0][0]
    ...
    def get_predictions(
        ...
        if apply_corr:
            corrs = self.get_corrections(
                mini=mini, feh=feh, eep=eep, corr_params=corr_params
            )
            dlogt, dlogr = corrs.T
            preds[:, self.logt_idx] += dlogt
            preds[:, self.logl_idx] += 2.0 * dlogr
    ...
    def get_corrections(self, mini=1.0, feh=0.0, eep=350.0, corr_params=None):
        """
        Returns interpolated corrections in some predictions for the input
        set of labels.
