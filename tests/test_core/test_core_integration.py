...
class TestCoreIntegration:
    ...
    def test_end_to_end_workflow(self, mist_integration_setup):
        ...
        assert -3.0 < best_labels['feh'] < 1.0  # Reasonable metallicity (MIST goes to -3)
        
        print(f"Best-fit parameters: mini={best_labels['mini']:.2f}, eep={best_labels['eep']:.0f}, feh={best_labels['feh']:.2f}")
    ...
    def test_performance_characteristics(self, mist_integration_setup):
        ...
        print(f"Performance: {time_per_model} s/model for {model_counts[:len(times)]} models")
    ...
    def test_grid_interpolation_accuracy(self, mist_integration_setup):
        ...
        if len(mini_vals) > 2 and len(eep_vals) > 2 and len(feh_vals) > 2:
            ...
            sed_interp, params_interp, _ = grid.get_seds(
                mini=mini_mid, eep=eep_mid, feh=feh_mid,
            ...
            print(f"Interpolation test: mini={mini_mid:.2f}, eep={eep_mid:.0f}, feh={feh_mid:.2f}")
...
class TestErrorHandling:
    ...
    def test_invalid_inputs(self, mist_integration_setup):
        ...
        with pytest.raises((ValueError, IndexError)):
            fitter.loglike_grid(
                obs[:5], obs_err, obs_mask,  # Wrong size
                verbose=False
            )
        ...
        with warnings.catch_warnings():
            ...
            try:
                result = fitter.loglike_grid(obs_bad, obs_err_bad, obs_mask_bad, verbose=False)
