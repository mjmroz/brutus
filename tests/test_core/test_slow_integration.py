...
class TestStarEvolTrackIntegration:
    ...
    def test_star_track_sed_generation(self, shared_star_track):
        ...
        try:
            ...
            assert isinstance(params, dict)  # params is a dictionary with stellar parameters
            assert len(sed) > 0
            ...
            assert all(np.isfinite(v) for v in param_values if isinstance(v, (int, float, np.number)))
    ...
    def test_star_track_multiple_seds(self, shared_star_track):
        ...
        test_cases = [
            {"mini": 0.8, "eep": 300, "feh": -0.5, "afe": 0.0, "av": 0.0, "dist": 500},   # Low-mass, metal-poor
            {"mini": 1.0, "eep": 350, "feh": 0.0, "afe": 0.0, "av": 0.1, "dist": 1000},   # Solar-type
            {"mini": 1.5, "eep": 400, "feh": 0.2, "afe": 0.0, "av": 0.05, "dist": 1500},  # Higher-mass, metal-rich
        ]
        ...
        for i, params in enumerate(test_cases):
            try:
                ...
                assert isinstance(sed, np.ndarray)
                assert isinstance(stellar_params, dict)  # stellar_params is a dictionary
                assert len(sed) > 0
                assert np.all(np.isfinite(sed))
        ...
        assert not np.array_equal(seds[0], seds[1]), "Different stellar parameters should yield different SEDs"
        assert not np.array_equal(seds[1], seds[2]), "Different stellar parameters should yield different SEDs"
        
        print(f"✅ Processed {len(results)} stellar configurations with varying results")
...
class TestNeuralNetworkIntegration:
    ...
    def test_neural_network_predictor_creation(self):
        ...
        try:
            ...
            print(f"✅ Neural network loaded successfully with {len(test_filters)} filters")
...
class TestFullPipeline:
    ...
    def test_end_to_end_individual_star(self, shared_star_track):
        ...
        try:
            ...
            mini, eep, feh, afe = 1.0, 350, 0.0, 0.0  # afe=0.0 always (no alpha variation in models)
            ...
            tiny_errors = np.full_like(sed, 0.001)  # Small errors for magnitude conversion
            ...
            print(f"🎉 End-to-end pipeline complete success: {len(mags)} magnitudes, T_eff={10**params['logt']:.0f}K")
